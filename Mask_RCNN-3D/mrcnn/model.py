...
def apply_box_deltas_graph_3D(boxes, deltas):
    """Applies the given deltas to the given boxes.
...
def clip_boxes_graph_3D(boxes, window):
    """
...
class DetectionLayer_3D(KE.Layer):
    """Takes classified proposal boxes and their bounding box deltas and
    returns the final detection boxes.
...
def load_image_gt_3D(dataset, config, image_id, augment=False, augmentation=None,
                  use_mini_mask=False):
    """Load and return ground truth data for an image (image, mask, bounding boxes).
...
def build_detection_targets_3D(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    """Generate targets for training Stage 2 classifier and mask heads.
    This is not used in normal training. It's useful for debugging or to train
    the Mask RCNN heads without using the RPN head.
...
def generate_random_rois_3D(image_shape, count, gt_class_ids, gt_boxes):
    """Generates ROI proposals similar to what a region proposal network
    would generate.
...
def data_generator_3D(dataset, config, shuffle=True, augment=False, augmentation=None,
                   random_rois=0, batch_size=1, detection_targets=False,
                   no_augmentation_sources=None):
    """A generator that returns images and corresponding target class ids,
    bounding box deltas, and masks.
...
def trim_zeros_graph_3D(boxes, name='trim_zeros'):
